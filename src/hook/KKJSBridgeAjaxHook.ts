/**
 * @file 妈网的离线包实现不需要这个文件，为了库完整性才保留在此
 */
...
export class _KKJSBridgeXHR {
    ...
    public static setProperties: Function = (response: any) => {
        ...
        const id: any = jsonObj.id;
        const xhr: any = _KKJSBridgeXHR.cache[id];
        if (xhr) {
            ...
            if (xhr.callbackProperties.readyState === xhr.DONE) {
                ...
                const load = document.createEvent('Events');
                load.initEvent('load');
    ...
    public static enableAjaxHook: Function = (enable: boolean) => {
        ...
        window._KKJSBridgeAjaxProxy.hookAjax({
            ...
            // 拦截方法
            open: function (arg: Array<any>, xhr: any) {
            ...
            send: function (arg: Array<any>, xhr: any) {
                console.log('send called:', arg[0]);
                const body: any = arg[0];
                const requestAsync: boolean = this.requestAsync;
                const bodyRequest: KK.AJAXBodySendRequest = {
                    id: this.id,
                    bodyType: 'String',
                    value: null
                };

                function sendBody(bodyRequest: KK.AJAXBodySendRequest, requestAsync = true) {
                    /*
					...
					*/
                    ...
                    // 同步 send 请求
                    const response: any = window.KKJSBridge.syncCall(_KKJSBridgeXHR.moduleName, 'send', bodyRequest);
                    // 处理请求回来的结果
            ...
            overrideMimeType: function (arg: Array<any>, xhr: any) {
                // console.log("overrideMimeType called:", arg[0]);
                _KKJSBridgeXHR.cacheXHRIfNeed(this);
                const mimetype: string = arg[0];
            ...
            abort: function (arg: Array<any>, xhr: any) {
            ...
            setRequestHeader: function (arg: Array<any>, xhr: any) {
                // console.log("setRequestHeader called:", arg[0], arg[1]);
                const headerName: string = arg[0];
                const headerValue: string = arg[1];
            ...
            getAllResponseHeaders: function (arg: Array<any>, xhr: any) {
                // console.log("getAllResponseHeaders called");
                let strHeaders = '';
                for (const name in this.callbackProperties.headers) {
                    strHeaders += `${name}: ${this.callbackProperties.headers[name]}\r\n`;
                }
                return strHeaders;
            },
            getResponseHeader: function (arg: Array<any>, xhr: any) {
                console.log('getResponseHeader called:', arg[0]);
                const headerName: string = arg[0];
                let strHeaders = '';
                const upperCaseHeaderName: string = headerName.toUpperCase();
                for (const name in this.callbackProperties.headers) {
