export class _KKJSBridgeFormData {
     ...
     * 遍历表单记录
     */
    public static traversalEntries: Function = (
        formData: any,
        traversal?: (key: string, value: any, fileName?: string) => void
    ) => {
        if (formData._entries) {
            // 低版本的 iOS 系统，并不支持 entries() 方法，所以这里做兼容处理
            for (let i = 0; i < formData._entries.length; i++) {
                const pair = formData._entries[i];
